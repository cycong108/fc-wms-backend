...
const { VendorDao } = require('../dao/vendor.dao')
...
class VendorService {
  async create(ctx) {
    ...
    const res = await VendorDao.create(model)
    if (res) {
  ...
  async update(ctx) {
    ...
    const res = await VendorDao.update(ctx.params.id, model)
    if (res) {
  ...
  async remove(ctx) {
    const res = await VendorDao.remove(ctx.params.id, ctx.state.user)
    if (res > 0) {
  ...
  async find(ctx) {
    const res = await VendorDao.find(ctx.request.query)
