...
class GoodsService {
  async create(ctx) {
    ...
    const res = await GoodsDao.create(model)
    if (res) {
  ...
  async update(ctx) {
    ...
    const res = await GoodsDao.update(ctx.params.id, model)
    if (res) {
  ...
  async remove(ctx) {
    const res = await GoodsDao.remove(ctx.params.id, ctx.state.user)
    if (res > 0) {
  ...
  async find(ctx) {
    const res = await GoodsDao.find(ctx.request.query)
